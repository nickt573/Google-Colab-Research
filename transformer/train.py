...
def get_ds(config):
    ...
    # Hard code in debugging, see original for adaptable code
...
def train_model(config, train_dataloader, val_dataloader, tokenizer_src, tokenizer_tgt):
    ...
    if model_filename:
        ...
        config['loss_history'] = state['loss_history']

        # Report loss history
        for i, loss in enumerate(config['loss_history']):
          print(f"Epoch {i} average loss: {loss:.4f}")
    else:
    ...
    for epoch in range(initial_epoch, max_epochs):
        ...
        for batch in batch_iterator:
            ...
            # Adding loss to global loss variable
        ...
        # Save and update the model at the end of every epoch
        old_filename = latest_weights_file_path(config)
        epoch_loss /= num_batches
        config['loss_history'].append(epoch_loss)
        print(f"Epoch {epoch} average loss: {epoch_loss:.4f}")

        if old_filename and os.path.exists(old_filename):
          os.remove(old_filename)
          print(f"{old_filename} updated to ", end='')
        else:
          print("Cannot find old file. Saving ", end='')
